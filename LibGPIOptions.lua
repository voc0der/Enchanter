...
local function GetSettingsCanvasWidth()
	if SettingsPanel and SettingsPanel.Container and SettingsPanel.Container.SettingsCanvas then
		return SettingsPanel.Container.SettingsCanvas:GetWidth()
	end
	if Options and Options.CurrentPanel and Options.CurrentPanel.GetWidth then
		local width = Options.CurrentPanel:GetWidth()
...
local addToBlizzardSettings = function(frame) -- alternative to the deprecated `InterfaceOptions_AddCategory`
	...
	elseif InterfaceOptions_AddCategory then
		...
		categoriesByName[frame.name] = frame
	end
end
...
local CreateScrollFrames = function(parent, name)
	if CreateScrollBoxLinearView and ScrollUtil and SettingsPanel then
		...
		scrollChild:SetFixedWidth(GetSettingsCanvasWidth() - scrollBar:GetWidth() - 8)
	...
	local scrollChild = CreateFrame("Frame", name.."ScrollChild")
	scrollFrame:SetScrollChild(scrollChild)
	scrollChild:SetSize(GetSettingsCanvasWidth() - 1, 100)
	function scrollChild.UpdateScrollLayout()
	end
...
function Options.OpenCategoryPanel(panelID)
	local panel = Options.CategoryPanels[panelID or 1]
	if not panel then
		return
	end
